...
With `OPA_AUTH_REQUIRED=1`, list routes (`GET /api/perf/scenarios`, `GET /api/perf/runs`) scope ClickHouse to `X-Organization-ID` / `X-Project-ID`. Omitting them returns empty lists (HTTP 200). See [interop.md](interop.md#tenant-headers).
